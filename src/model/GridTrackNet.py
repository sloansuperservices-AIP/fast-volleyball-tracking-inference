from tensorflow.keras.layers import Activation, BatchNormalization, Conv2D, Input, MaxPooling2D
from tensorflow.keras.models import Model


def GridTrackNet(imgs_per_instance, input_height, input_width):
    imgs_input = Input(shape=(input_height, input_width, imgs_per_instance * 3))
    ...
    x = Conv2D(imgs_per_instance * 3, (3, 3), kernel_initializer="random_uniform", padding="same")(x)
    output = Activation("sigmoid")(x)

    return Model(imgs_input, output)


def GridTrackNetLegacy(imgs_per_instance, input_height, input_width):
