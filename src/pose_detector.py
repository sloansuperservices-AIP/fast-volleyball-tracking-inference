...
def main():
    ...
    if args.track_file:
        if os.path.exists(args.track_file):
            add_pose_to_track_json(args.track_file, args.video_path, output_dir=args.output_dir, visualize=args.visualize)
        else:
            print(f"Track file not found: {args.track_file}")
    else:
        print("Please provide --track_file for pose detection.")
