...
class TrackProcessor:
    ...
    def _load_tracks_from_json(self) -> None:
        ...
        json_files = sorted(
            [f for f in os.listdir(self.json_dir) if f.startswith("track_") and f.endswith(".json")]
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Visualize or export ball tracking data with FPS monitoring")
    parser.add_argument("--json_dir", type=str, default=None, help="Directory with track_*.json files")
    parser.add_argument("--video_path", type=str, required=True, help="Path to source video")
    parser.add_argument("--output_path", type=str, default=None, help="Combined output video (MP4)")
    parser.add_argument(
        "--split_dir",
        type=str,
        default=None,
