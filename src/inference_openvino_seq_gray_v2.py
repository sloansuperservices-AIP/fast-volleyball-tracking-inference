...
def load_model(model_path, device="CPU"):
    if Core is None:
        raise ImportError("openvino package not installed")
...
def render_prediction(frame, visibility, x_orig, y_orig, writer, visualize, track, max_missing):
    if visibility:
        track.append((x_orig, y_orig))
        track._missing = 0
    else:
        track._missing = getattr(track, '_missing', 0) + 1
        if track._missing >= max_missing:
            track.clear()
            track._missing = 0
    ...
    try:
        while cap.isOpened():
            ...
            for local_index, (frame_item, prediction) in enumerate(
                ...
                render_prediction(
                    frame_item, visibility, x_orig, y_orig, writer, args.visualize, track, args.track_length
                )
        ...
        if current_frames:
            ...
            for local_index, frame_item in enumerate(current_frames):
                ...
                append_to_csv(result, csv_path)
                render_prediction(frame_item, 0, -1, -1, writer, args.visualize, track, args.track_length)
