...
def load_model(model_path, device="CPU"):
    ...
    if pshape.is_dynamic:
        print(f"Dynamic input shape detected, fixing to {expected_shape}")
...
def run_tracking_ov(
    ...
    compiled_model, _, output_layer, model_params = load_model(model_xml, device=device)
    cap, frame_width, frame_height, fps, total = initialize_video(video_path)

    basename = os.path.splitext(os.path.basename(video_path))[0]
    writer, _ = setup_output_writer(basename, output_dir, frame_width, frame_height, fps, only_csv)
    ...
    try:
        while cap.isOpened():
            ...
            predictions = decode_predictions(output, model_params, threshold)
            ...
            for local_index, (frame_item, prediction) in enumerate(zip(current_frames, predictions)):
                ...
                render_prediction(frame_item, visibility, x_orig, y_orig, writer, visualize, track)
        ...
        if current_frames:
            ...
            for local_index, frame_item in enumerate(current_frames):
                ...
                append_to_csv(result, csv_path)
                render_prediction(frame_item, 0, -1, -1, writer, visualize, track)
