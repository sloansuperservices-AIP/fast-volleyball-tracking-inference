...
def setup_output_writer(basename, out_dir, width, height, fps, only_csv):
    if out_dir is None or only_csv:
        return None, None
    video_dir = os.path.join(out_dir, basename)
    os.makedirs(video_dir, exist_ok=True)
    path = os.path.join(video_dir, "predict.mp4")
...
def setup_csv_file(basename, out_dir):
    ...
    video_dir = os.path.join(out_dir, basename)
    os.makedirs(video_dir, exist_ok=True)
    path = os.path.join(video_dir, "ball.csv")
...
def render_prediction(frame, visibility, x_orig, y_orig, radius, contour, writer, visualize, track):
    if visibility:
...
def main():
    ...
    writer, _ = setup_output_writer(basename, args.output_dir, frame_width, frame_height, fps, args.only_csv)
    ...
    try:
        while cap.isOpened():
            ...
            if len(current_frames) != seq:
                ...
                pbar.update(1)
                continue
            processed_frames = preprocess_frames(current_frames, model_params["input_height"], model_params["input_width"])
            ...
            for local_index, (frame_item, prediction) in enumerate(zip(current_frames, predictions)):
                ...
                if visibility:
                    ...
                    radius, contour = estimate_ball_radius(prev_gray, gray_frame, x_orig, y_orig, size_state)
                else:
                    x_orig, y_orig = -1, -1
                ...
                visualization_enabled = render_prediction(frame_item, visibility, x_orig, y_orig, radius, contour, writer, visualization_enabled, track)
