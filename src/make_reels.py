...
WATERMARK_FONT_PATH = (
    Path(__file__).resolve().parent / "fonts" / "PlayfairDisplay-MediumItalic.ttf"
)
...
def smooth_values(
    values: np.ndarray, method: str, window: int, polyorder: int
) -> np.ndarray:
    if method == "none":
        return values
    ...
    if method == "savitzky_golay":
        ...
        return savgol_filter(
            values, window_length=window, polyorder=polyorder, mode="interp"
        )
    if method == "kalman":
...
def kalman_smooth(
    values: np.ndarray, process_var: float = 1e-3, meas_var: float = 1e-1
) -> np.ndarray:
    if len(values) == 0:
        return values
    x_est = values[0]
...
def crop_frame(
    frame: np.ndarray, center_x: int, crop_width: int, padding: str
) -> np.ndarray:
...
def add_watermark_top_right(frame: np.ndarray, text: str = WATERMARK_TEXT) -> np.ndarray:
    ...
    cv2.putText(
        fallback_overlay, text, (x, y), font, scale, (255, 255, 255), thickness, cv2.LINE_AA
    )
...
def crop_and_save_track_payloads(
    ...
    with tempfile.TemporaryDirectory(prefix="reel_segments_") as tmp_dir_raw:
        ...
        try:
            subprocess.run(
                concat_cmd, cwd=tmp_dir, check=True, capture_output=True, text=True
            )
