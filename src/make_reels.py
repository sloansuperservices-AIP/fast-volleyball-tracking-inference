...
def add_watermark_top_right(frame: np.ndarray, text: str = WATERMARK_TEXT) -> np.ndarray:
    ...
    cv2.putText(fallback_overlay, text, (x + 2, y + 2), font, scale, (0, 0, 0), thickness + 2, cv2.LINE_AA)
...
def crop_and_save_track(
    ...
    for i, frame_idx in enumerate(tqdm(range(start_frame, end_frame + 1), desc="Saving frames", total=total_frames, unit="frame")):
        ret, frame = cap.read()
        if not ret:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Visualize rally clips with ball-centered cropping.")
    ...
    parser.add_argument("--smooth_window", type=int, default=DEFAULT_SMOOTH_WINDOW, help="Window size for smoothing")
    parser.add_argument("--smooth_polyorder", type=int, default=2, help="Polynomial order for Savitzky-Golay")
    parser.add_argument("--interpolation", choices=["hold", "linear"], default="hold", help="Interpolation method")
    parser.add_argument("--margin", type=float, default=0.0, help="Lead offset in pixels")
    parser.add_argument("--padding", choices=["none", "mirror", "black"], default="none", help="Padding strategy")
...
def main() -> None:
    ...
    if args.track_jsons:
        ...
        json_paths = sorted(
            [os.path.join(args.json_dir, f) for f in os.listdir(args.json_dir) if f.startswith("track_") and f.endswith(".json")]
        )
    else:
