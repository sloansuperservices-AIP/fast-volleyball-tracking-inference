...
class TrackAnalyzer:
    """Class for analyzing tracks from CSV and visualizing them on video."""
    def __init__(
        self,
        csv_path: str,
        video_path: str,
    ...
    def _get_cache_key(self) -> str:
        """Creates a hash based on parameters and CSV file content."""
        params = {
    ...
    def _trim_bounce_start(self, track: Dict) -> Dict:
        """Trims the beginning of the track, removing frames with up-and-down ball movement."""
        ...
        for i, track1 in enumerate(sorted_tracks):
            ...
            for j, track2 in enumerate(sorted_tracks):
                ...
                if self._is_overlapping(track1, track2):
                    used_indices.add(j)
                    print(
                        f"Removed track {track2.track_id} (overlaps with track {track1.track_id})"
                    )
    ...
    def _print_track_info(self) -> None:
        print("\n=== Track List ===")
        print(f"Minimum track duration: {self.min_duration_sec} sec")
        ...
        if not self.tracks:
            print("No tracks satisfying criteria found.")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=200,
        help="Maximum tracking distance",
    )
    parser.add_argument(
        "--min_duration_sec",
        type=float,
        default=1.0,
        help="Minimum track duration (sec)",
    )
    parser.add_argument(
        "--max_x_displacement",
        type=float,
        default=20.0,
        help="Maximum X displacement for lob detection (pixels)",
    )
    parser.add_argument(
        "--min_y_displacement",
        type=float,
        default=50.0,
        help="Minimum Y displacement for lob detection (pixels)",
    )
    parser.add_argument(
        "--bounce_frames",
        type=int,
        default=10,
        help="Number of frames for lob analysis",
    )
