...
class TrackCalculator:
    ...
    def _calculate_cm_per_px_scale(self) -> Optional[float]:
        ...
        # Point mapping: 1..4 corners, 5..6 center, 7..8 net top
    ...
    def _maybe_rescale_court_geometry(self, df: pd.DataFrame) -> None:
        ...
        if target_w is None or target_h is None:
            ...
            if (
                pd.notna(max_x)
                and pd.notna(max_y)
                and (max_x > self._court_geometry.image_width or max_y > self._court_geometry.image_height)
            ):
                target_w = max(int(max_x) + 1, self._court_geometry.image_width)
                target_h = max(int(max_y) + 1, self._court_geometry.image_height)
                LOG.warning(
                    "Detected coordinates exceed court.json image size (%sx%s). Rescaling to %sx%s.",
    ...
    def _classify_camera_position(self) -> str:
        ...
        dy = p8[1] - p7[1]
        court_span = max(
            np.hypot(p4[0] - p1[0], p4[1] - p1[1]), np.hypot(p3[0] - p2[0], p3[1] - p2[1]), 1.0
        )
    ...
    def _find_rolling_start_frame(self, positions: List[Any], start_index: int = 0) -> Optional[int]:
    ...
    def _trim_after_game_pause(self, track: Track) -> Track:
        ...
        if pause_frame is None or pause_frame <= track.start_frame or pause_frame >= track.last_frame:
            return track
    ...
    def _merge_overlapping(self, tracks: List[Track]) -> List[Track]:
        ...
        def _can_merge(track1: Track, track2: Track) -> bool:
            b1, b2 = _frame_bounds(track1), _frame_bounds(track2)
    ...
    def _split_discontinuous_tracks(self, tracks: List[Track]) -> List[Track]:
        ...
        for track in tracks:
            ...
            for pos in positions[1:]:
                if int(pos[1]) - int(chunks[-1][-1][1]) > MAX_MERGE_GAP_FRAMES:
                    chunks.append([pos])
                else:
                    chunks[-1].append(pos)
            if len(chunks) == 1:
            ...
            for idx, chunk in enumerate(chunks):
                child = Track()
                child.track_id, child.reason, child.fps = track.track_id * 1000 + idx, track.reason, track.fps
                child.positions = chunk
                child.start_frame, child.last_frame = int(chunk[0][1]), int(chunk[-1][1])
    ...
    def _filter_pause_rollings(self, tracks: List[Track]) -> List[Track]:
        ...
        for track in tracks:
            analysis = self._analyze_track_trajectory(track)
            if analysis["rolling_start_frame"] is not None and analysis["last_above_net_frame"] is None:
                LOG.debug("Filtered pause rolling track: id=%s", track.track_id)
                continue
    ...
    def _process_detections(self, df: pd.DataFrame) -> None:
        tracker = BallTracker(
            buffer_size=2500, max_disappeared=40, max_distance=self._scaled_max_distance, fps=self.config.fps
        )
        ...
        for track in self.tracks:
            ...
            if self._court_enabled:
                ...
                for pos in track_dict["positions"]:
                    court_x, court_y = self._coordinate_transformer.to_court(pos[0][0], pos[0][1])
...
def main() -> None:
    ...
    parser.add_argument("--court_json_path", type=str, help="Path to court JSON file")
    ...
    parser.add_argument("--bounce_frames", type=int, default=DEFAULT_BOUNCE_FRAMES, help="Bounce frames")
