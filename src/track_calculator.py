...
class TrackCalculator:
    ...
    def _maybe_rescale_court_geometry(self, df: pd.DataFrame) -> None:
        ...
        if target_w is None or target_h is None:
            ...
            if (pd.notna(max_x) and pd.notna(max_y) and (max_x > self._court_geometry.image_width or max_y > self._court_geometry.image_height)):
                ...
                LOG.warning("Detected coordinates exceed court.json image size. Applied best-effort scaling.")
    ...
    def _analyze_track_trajectory(self, track: Track) -> Dict[str, Any]:
        ...
        stop_rising_sec = (float(stop_rising_frame) / self.config.fps if stop_rising_frame is not None and self.config.fps > 0 else None)
        return {
            "camera_position": self._camera_position,
    ...
    def _filter_pause_rollings(self, tracks: List[Track]) -> List[Track]:
        ...
        for track in tracks:
            analysis = self._analyze_track_trajectory(track)
            is_pause_rolling = (analysis["rolling_start_frame"] is not None and analysis["last_above_net_frame"] is None)
            if is_pause_rolling:
                continue
    ...
    def _process_detections(self, df: pd.DataFrame) -> None:
        tracker = BallTracker(buffer_size=2500, max_disappeared=40, max_distance=self._scaled_max_distance, fps=self.config.fps)
...
def main() -> None:
    ...
    parser.add_argument("--max_distance", type=float, default=DEFAULT_MAX_DISTANCE)
    parser.add_argument("--min_duration_sec", type=float, default=DEFAULT_MIN_DURATION_SEC)
    parser.add_argument("--max_x_displacement", type=float, default=DEFAULT_MAX_X_DISPLACEMENT)
    parser.add_argument("--min_y_displacement", type=float, default=DEFAULT_MIN_Y_DISPLACEMENT)
    parser.add_argument("--bounce_frames", type=int, default=DEFAULT_BOUNCE_FRAMES)
