...
class TrackCalculator:
    def __init__(self, config: TrackCalculatorConfig) -> None:
        ...
        if config.court_json_path and not self._court_geometry:
            LOG.warning(
                "Court JSON provided but could not be loaded, using image coordinates"
            )
        else:
    ...
    def _maybe_rescale_court_geometry(self, df: pd.DataFrame) -> None:
        ...
        scaled_keypoints = tuple(
            (x * scale_x, y * scale_y) for x, y in self._court_geometry.keypoints
        )
    ...
    def _classify_camera_position(self) -> str:
        ...
        depth_ratio = max(left_depth, right_depth) / max(
            1.0, min(left_depth, right_depth)
        )
    ...
    def _is_above_net(self, x: float, y: float) -> bool:
        ...
        image_h = (
            self._court_geometry.image_height if self._court_geometry else 720
        )
    ...
    def _interpolate_gaps(self, df: pd.DataFrame, max_gap: int = 15) -> pd.DataFrame:
        """Fills short visibility gaps in ball detection CSV."""
        if df.empty:
            return df
        df = df.copy().sort_values("Frame")
        df["X"] = pd.to_numeric(df["X"])
        df["Y"] = pd.to_numeric(df["Y"])
        df["Visibility"] = pd.to_numeric(df["Visibility"])

        # We only interpolate if there's a gap between Visibility=1
        for i in range(1, len(df)):
            if df.iloc[i]["Visibility"] == 0 and df.iloc[i-1]["Visibility"] > 0:
                start_idx = i - 1
                start_frame = df.iloc[start_idx]["Frame"]

                # Look for end of gap
                end_idx = None
                for j in range(i, min(i + max_gap + 1, len(df))):
                    if df.iloc[j]["Visibility"] > 0:
                        end_idx = j
                        break

                if end_idx is not None:
                    end_frame = df.iloc[end_idx]["Frame"]
                    gap_len = end_frame - start_frame
                    if gap_len <= max_gap + 1:
                        # Linear interpolation
                        start_x, start_y = df.iloc[start_idx]["X"], df.iloc[start_idx]["Y"]
                        end_x, end_y = df.iloc[end_idx]["X"], df.iloc[end_idx]["Y"]
                        for k in range(start_idx + 1, end_idx):
                            t = (df.iloc[k]["Frame"] - start_frame) / gap_len
                            df.iloc[k, df.columns.get_loc("X")] = start_x + t * (end_x - start_x)
                            df.iloc[k, df.columns.get_loc("Y")] = start_y + t * (end_y - start_y)
                            df.iloc[k, df.columns.get_loc("Visibility")] = 0.5  # Mark as interpolated
        return df
    ...
    def _process_detections(self, df: pd.DataFrame) -> None:
        ...
        close_reason_timeout = f"inactivity_timeout: no detections for >{tracker.max_disappeared} frames"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--video_width", type=int, default=1920, help="Source video width")
    parser.add_argument("--video_height", type=int, default=1080, help="Source video height")
    ...
    parser.add_argument("--min_duration_sec", type=float, default=DEFAULT_MIN_DURATION_SEC, help="Min track duration")
    parser.add_argument("--max_x_displacement", type=float, default=DEFAULT_MAX_X_DISPLACEMENT, help="Max X displacement")
    parser.add_argument("--min_y_displacement", type=float, default=DEFAULT_MIN_Y_DISPLACEMENT, help="Min Y displacement")
    parser.add_argument("--bounce_frames", type=int, default=DEFAULT_BOUNCE_FRAMES, help="Bounce analysis frames")
