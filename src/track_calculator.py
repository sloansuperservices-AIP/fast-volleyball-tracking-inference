...
class TrackCalculator:
    ...
    def _process_detections(self, df: pd.DataFrame) -> None:
        ...
        # Performance optimization: use groupby instead of O(N^2) filtering
        frame_groups = df.groupby('Frame')

        for frame_num in all_frames:
            if frame_num not in frame_groups.groups:
                continue
            frame_rows = frame_groups.get_group(frame_num)

            detections = []
            for row in frame_rows.itertuples(index=False):
                if not np.isnan(row.X) and not np.isnan(row.Y):
                    # radius fallback if missing
                    radius = getattr(row, 'Radius', DEFAULT_DETECTION_BOX_RADIUS)
                    if pd.isna(radius): radius = DEFAULT_DETECTION_BOX_RADIUS
