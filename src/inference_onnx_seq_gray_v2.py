...
def _update_ld_library_path():
    """Dynamically find and append Nvidia CUDA libraries to LD_LIBRARY_PATH if they exist in .venv."""
    import sys

    # Try to locate the site-packages in the current virtual environment
    site_packages = next((p for p in sys.path if "site-packages" in p and ".venv" in p), None)
    if site_packages:
        cublas_lib = Path(site_packages) / "nvidia" / "cublas" / "lib"
        if cublas_lib.exists():
            os.environ["LD_LIBRARY_PATH"] = str(cublas_lib) + ":" + os.environ.get("LD_LIBRARY_PATH", "")
