...
def _update_ld_library_path():
    """
    Dynamically update LD_LIBRARY_PATH to include nvidia cublas if present in site-packages.
    This helps onnxruntime find necessary libraries without hardcoding virtualenv paths.
    """
    try:
        import site
        packages = site.getsitepackages()
        for p in packages:
            cublas_path = Path(p) / "nvidia" / "cublas" / "lib"
            if cublas_path.exists():
                os.environ["LD_LIBRARY_PATH"] = str(cublas_path) + ":" + os.environ.get("LD_LIBRARY_PATH", "")
                break
    except Exception:
        pass
