...
def setup_cuda_env():
    # Attempt to locate nvidia/cublas/lib in .venv
    venv_base = Path("./.venv/lib")
    if venv_base.exists():
        py_dirs = list(venv_base.glob("python3.*"))
        if py_dirs:
            # Take the first one or logic based on current sys.version
            cublas_lib = py_dirs[0] / "site-packages/nvidia/cublas/lib"
