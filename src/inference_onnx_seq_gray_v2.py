...
def postprocess_heatmap_output(
    ...
    results = []
    # If output is flattened (1, out_dim * H * W), reshape it
    if len(output.shape) == 2:
        output = output.reshape(1, out_dim, input_height, input_width)
...
def postprocess_grid_output(
    ...
    if len(output.shape) == 2:
        output = output.reshape(1, seq, 3, grid_rows, grid_cols)
