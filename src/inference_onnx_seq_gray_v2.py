...
# Attempt to locate and add Nvidia libraries to LD_LIBRARY_PATH if needed
try:
    import site
    for path in site.getsitepackages():
        cuda_path = os.path.join(path, "nvidia/cublas/lib")
...
def infer_model_params(model_path: str) -> dict:
    ...
    if "vballnetgrid" in model_name or "grid" in model_name:
        return {
            "family": "grid",
            "seq": 9,
            "input_seq": 9,
