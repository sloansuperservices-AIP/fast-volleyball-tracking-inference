...
def _update_ld_library_path():
    """Dynamically find and add Nvidia CUDA libraries to LD_LIBRARY_PATH."""
    venv_base = Path(".venv")
    if venv_base.exists():
        for lib_path in venv_base.glob("**/nvidia/*/lib"):
            if lib_path.is_dir():
                os.environ["LD_LIBRARY_PATH"] = f"{lib_path.absolute()}:{os.environ.get('LD_LIBRARY_PATH', '')}"
                LOG.debug(f"Added {lib_path} to LD_LIBRARY_PATH")

_update_ld_library_path()
# Preload DLLs for ONNX Runtime if on Windows, but this is usually safe on Linux too if the method exists
