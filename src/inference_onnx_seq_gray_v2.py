...
try:
    ...
    class BallTrack:
        ...
        def update(self, point):
            if len(self._points) >= self.maxlen: self._points.pop(0)
            self._points.append(point)
        def reset(self): self._points.clear()
        def points(self): return [p for p in self._points if p is not None]

# ort.preload_dlls() # Not available in all environments
