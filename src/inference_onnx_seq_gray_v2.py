...
# Optional: ensure Nvidia libraries are in path if using CUDA
if os.path.exists("./.venv"):
    for root, dirs, files in os.walk("./.venv"):
        if "nvidia/cublas/lib" in root:
...
def render_prediction(
    frame, points, visibility, x_orig, y_orig, radius, contour
):
...
def run_inference(session, input_tensor, has_gru, h0, input_names, output_names):
    ...
    if has_gru:
        if len(outputs) < 2:
            raise ValueError(
                "GRU model should output at least 2 values: heatmaps and hn"
            )
        new_h0 = outputs[1]
...
def main():
    ...
    frame_buffer = []
    track_state = BallTrackState(
        maxlen=args.track_length, max_missing=args.track_length
    )
    frame_index = 0
    ...
    try:
        while not stop_event.is_set():
            ...
            predictions = decode_predictions(
                output, model_params, args.confidence_threshold
            )
