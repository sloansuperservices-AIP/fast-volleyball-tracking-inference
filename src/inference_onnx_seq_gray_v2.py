...
# Attempt to locate Nvidia CUDA libs within the .venv and add them to LD_LIBRARY_PATH
# This allows onnxruntime-gpu to find its dependencies in restricted environments.
def _setup_cuda_libs():
    venv_site_packages = Path("./.venv/lib")
    if venv_site_packages.exists():
        # Search for nvidia/cublas/lib or similar paths
        cuda_paths = list(venv_site_packages.glob("**/nvidia/*/lib"))
        if cuda_paths:
            extra_ld_path = ":".join(str(p.resolve()) for p in cuda_paths)
            os.environ["LD_LIBRARY_PATH"] = f"{extra_ld_path}:{os.environ.get('LD_LIBRARY_PATH', '')}"
            LOG.debug("Augmented LD_LIBRARY_PATH with CUDA libs: %s", extra_ld_path)
...
def load_onnx_model(model_path):
    ...
    # Update model_params from actual ONNX shape if known
    model_params["input_seq"] = resolve_dim(input_shape[1], model_params["input_seq"])

    if model_params["family"] == "grid":
        output_channels = resolve_dim(output_shape[1], model_params["input_seq"] * 3)
        model_params["seq"] = output_channels // 3
    else:
        model_params["seq"] = resolve_dim(output_shape[1], model_params["seq"])
    ...
    return session, has_gru, model_params["seq"], h0_shape, batch_size, input_names, output_names, model_params
...
def postprocess_grid_output(output, threshold, seq, input_height, input_width, grid_rows, grid_cols):
        ...
        while not stop_event.is_set():
            ...
            for i, (visibility, x, y) in enumerate(predictions[: len(frames)]):
                ...
                if visibility:
                    ...
                    radius, contour = estimate_ball_radius(prev_gray, frame_gray, point[0], point[1], size_state)
                else:
                ...
                if visualization_enabled or out_writer is not None:
                    ...
                    if visualization_enabled:
                        visualization_enabled, should_exit = show_visualization(visualization_enabled, vis_frame)
                        if should_exit:
                            exit_flag = True
                            stop_event.set()
                            break
