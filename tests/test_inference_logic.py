...
class TestInferenceLogic(unittest.TestCase):
    def test_postprocess_heatmap(self):
        ...
        results = postprocess_output(output, threshold=0.5, input_height=100, input_width=100, out_dim=3, is_grid=False)
    ...
    def test_postprocess_grid(self):
        # Create a mock grid output (batch=1, seq*3=3, grid_h=2, grid_w=2)
        # Sequence length = 1
        ...
        results = postprocess_output(output, threshold=0.5, input_height=100, input_width=100, out_dim=3, is_grid=True)
