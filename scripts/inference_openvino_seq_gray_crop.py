...
def parse_args():
    ...
    parser.add_argument("--action_crop_size", type=int, default=ACTION_CROP_SIZE, help="Output action crop size")
    parser.add_argument("--ball_circumference_cm", type=float, default=BALL_CIRCUMFERENCE_CM, help="Real ball circumference in cm")
    parser.add_argument("--player_height_cm", type=float, default=PLAYER_HEIGHT_CM, help="Player height used to size the action crop in cm")
...
def load_model(model_path, device="CPU"):
    ...
    if not pshape.is_dynamic and len(pshape) >= 2:
        model_params["seq"] = int(pshape[1].get_length())
    expected_shape = [
        1,
        model_params["seq"],
...
def ball_radius_to_cm_per_px(radius_px, ball_circumference_cm):
    if radius_px <= 0 or ball_circumference_cm <= 0:
        return None
    ball_diameter_cm = ball_circumference_cm / np.pi
    return ball_diameter_cm / (2.0 * float(radius_px))


def compute_action_crop_rect(
    detections, frame_width, frame_height, ball_circumference_cm, player_height_cm
):
    visible = [
        item
        for item in detections
        if item["Visibility"] and item["X"] >= 0 and item["Y"] >= 0
    ]
    if not visible:
        return 0, 0, min(frame_width, frame_height), 1.0

    radii = [item["Radius"] for item in visible if item["Radius"] > 0]
    ...
    if cm_per_px is None:
        side_by_player = min(frame_width, frame_height)
    else:
        side_by_player = int(round(player_height_cm / cm_per_px))

    xs = np.array([item["X"] for item in visible], dtype=np.float32)
    ys = np.array([item["Y"] for item in visible], dtype=np.float32)
    min_x, max_x = float(xs.min()), float(xs.max())
    min_y, max_y = float(ys.min()), float(ys.max())

    top_context = side_by_player * BALL_TOP_PLAYER_CONTEXT_RATIO
    bottom_context = side_by_player * BALL_BOTTOM_PLAYER_CONTEXT_RATIO

    # One crop is shared by the whole sequence. In volleyball the ball is often
    # near shoulder height, so keep less sky above the ball path and reserve
    # more context below for the player body and feet.
    side = max(side_by_player, int(round(max_x - min_x + side_by_player)))
    side = max(side, int(round(max_y - min_y + top_context + bottom_context)))
    side = max(1, min(side, max(frame_width, frame_height)))

    center_x = (min_x + max_x) / 2.0
    left = int(round(center_x - side / 2.0))
    top = int(round(min_y - top_context))
    left = max(0, min(left, max(0, frame_width - side)))
    top = max(0, min(top, max(0, frame_height - side)))
    return left, top, side, cm_per_px or 1.0


def crop_gray_square(frame, crop_rect, crop_size):
    left, top, side, _ = crop_rect
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
...
def transform_point_to_crop(x, y, crop_rect, crop_size):
    left, top, side, _ = crop_rect
    if x < 0 or y < 0 or side <= 0:
        return None
    scale = float(crop_size) / float(side)
    cx = int(round((x - left) * scale))
    cy = int(round((y - top) * scale))
    if cx < 0 or cy < 0 or cx >= crop_size or cy >= crop_size:
        return None
    return cx, cy


def transform_contour_to_crop(contour, crop_rect, crop_size):
    if contour is None:
        return None
    left, top, side, _ = crop_rect
    if side <= 0:
        return None
    transformed = contour.astype(np.float32).copy()
    transformed[:, :, 0] = (transformed[:, :, 0] - left) * crop_size / side
    transformed[:, :, 1] = (transformed[:, :, 1] - top) * crop_size / side
...
def render_prediction(
    ...
    if writer or visualize:
        vis_frame = crop_gray_square(frame, crop_rect, crop_size)
        crop_track = deque(
            (
                point
                for point in (
                    transform_point_to_crop(px, py, crop_rect, crop_size)
                    for px, py in track
                )
        ...
        vis_frame = draw_track(vis_frame, crop_track)
        crop_point = transform_point_to_crop(x_orig, y_orig, crop_rect, crop_size)
        if visibility and crop_point is not None:
            crop_contour = transform_contour_to_crop(contour, crop_rect, crop_size)
            if crop_contour is not None:
                cv2.drawContours(vis_frame, [crop_contour], -1, (0, 255, 255), 1)
            draw_radius = int(round(radius * crop_size / crop_rect[2])) if radius > 0 else 8
            draw_radius = max(2, draw_radius)
            cv2.circle(vis_frame, crop_point, draw_radius, (0, 255, 0), 2)
            cv2.putText(
                vis_frame,
                f"R:{radius}" if radius > 0 else "R:n/a",
                (min(crop_size - 64, crop_point[0] + draw_radius + 4), max(18, crop_point[1] - 10)),
...
def main():
    ...
    writer, _ = setup_output_writer(
        basename, args.output_dir, args.action_crop_size, args.action_crop_size, fps, args.only_csv
    )
    ...
    try:
        while cap.isOpened():
            ...
            for local_index, (frame_item, prediction) in enumerate(
                ...
                batch_results.append((result, contour))
                prev_gray = gray_frame

            crop_rect = compute_action_crop_rect(
                [result for result, _ in batch_results],
            ...
            for frame_item, (result, contour) in zip(current_frames, batch_results, strict=True):
        ...
        if current_frames:
            ...
            for local_index, frame_item in enumerate(current_frames):
                ...
                append_to_csv(result, csv_path)
                crop_side = min(frame_width, frame_height)
                crop_rect = (
                    max(0, (frame_width - crop_side) // 2),
                    max(0, (frame_height - crop_side) // 2),
                    crop_side,
                    1.0,
                )
                visualization_enabled = render_prediction(
